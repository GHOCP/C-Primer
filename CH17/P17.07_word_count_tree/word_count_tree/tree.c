...
/** || local data type **/
/*
    we designed SeekItem() to return a structure containing two pointers:
    one pointing to the node containing the item ( NULL if the item isn't found) and one pointing
    to the parent node ( NULL if the node is the root and has no parent).
*/
typedef struct pair {
    Trnode * parent;
    Trnode * child;
...
bool AddItem(const Item * pi, Tree * ptree) {
    ...
    if (SeekItem(pi, ptree).child != NULL) {
        SeekItem(pi, ptree).child->item.num++;
...
/* thus *ptr point to the target node */
{
    Trnode * temp;
    if ( (*ptr)->left == NULL) {
        temp = *ptr;
